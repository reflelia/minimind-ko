...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # forward 전파（student 모델）
        with autocast_ctx:
            res = model(input_ids)
            student_logits = res.logits[..., :-1, :].contiguous()

        # teacher 모델forward 전파（eval 및 no_grad에서만 사용）
        if teacher_model is not None:
            with torch.no_grad():
...
if __name__ == "__main__":
    # 예시는 MoE 모델로 dense 모델을 distillation합니다. 더 큰 teacher 모델로 더 작은 student 모델을 distillation할 수도 있습니다.
