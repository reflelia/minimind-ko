...
# ======== mock 데이터 ========
WEATHER_DATA = {"서울": ("28°C", "맑음"), "부산": ("15°C", "흐림"), "제주": ("32°C", "무더움"), "대구": ("30°C", "맑음"), "인천": ("22°C", "흐림"), "광주": ("18°C", "비"), "대전": ("25°C", "흐림"), "울산": ("20°C", "맑음"), "수원": ("16°C", "바람"), "창원": ("26°C", "흐림"), "Tokyo": ("12°C", "맑음"), "New York": ("8°C", "흐림"), "London": ("5°C", "비"), "Paris": ("10°C", "흐림"), "Sydney": ("25°C", "맑음")}
...
TRANSLATE_DATA = {("안녕하세요", "english"): "Hello World", ("Good morning", "korean"): "좋은 아침입니다", ("오늘 날씨가 정말 좋네요", "english"): "The weather is nice today", ("I love programming", "korean"): "저는 프로그래밍을 좋아합니다", ("기계 학습은 흥미롭습니다", "english"): "Machine learning is interesting", ("Happy birthday", "korean"): "생일 축하합니다"}
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    ...
    for turn in range(max_turns):
        ...
        for call in calls:
            ...
            result_str = (json.dumps(result, ensure_ascii=False) if result else '{"error": "tool not found"}')[:2048]  # 너무 긴 숫자로 tokenizer가 과도하게 커지는 것을 방지합니다.
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None):
    ...
    for idx, response in enumerate(completions):
        ...
        reward -= 0.5 * sum(abs(turn.count('<tool_call>') - turn.count('</tool_call>')) for turn in turn_answers)  # 태그 불일치 penalty
        # -------- 도구 호출 없음: 형식 + reward 점수 --------
        if not tool_calls:
            reward += 0.5 if 5 <= len(response.strip()) <= 800 else -0.5  # 길이 점수
            if '</think>' in response:
                think, answer = response.split('</think>', 1)
                reward += 1.0 if 20 <= len(think.strip()) <= 300 else -0.5  # Thinking길이 점수
                reward += 0.25 if response.count('</think>') == 1 else -0.25  # thinking 닫힘 점수
                answer = answer.strip()
            ...
            rewards[idx] = max(min(reward, 3.0), -3.0)  # 총점 clip
        # -------- 도구 호출 있음: 실행 결과 점수 --------
        else:
            ...
            tool_gap = abs(valid_call_count - len(gt)) + max(0, len(tool_calls) - valid_call_count)  # tool 개수 차이
            ...
            if unfinished: reward -= 0.5  # 미완료 penalty
