...
# 사용자 정의 Critic 모델. MiniMindLM을 상속합니다.
class CriticModel(MiniMindForCausalLM):
    def __init__(self, params):
        super().__init__(params)
        # lm_head를 단일 value를 출력하는 linear layer로 교체합니다.
...
def ppo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, actor_scheduler, critic_scheduler, reward_model, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        for ppo_epoch in range(args.ppo_update_iters):
            ...
            for i in range(0, B, mb_size):
                ...
                with autocast_ctx:
                    ...
                    # autocast 안에서 log_softmax를 계산해 fp16/bf16 logits에 대한
                    # 직접 계산으로 생기는 추가 수치 오차를 피합니다.
                ...
                # 선택적 진단: 첫 epoch 첫 minibatch의 현재 logp와 old logp 차이를 관찰합니다.
                ...
                # 각 GPU의 approx_kl을 동기화해 일부 rank만 break하면서 DDP deadlock이 생기는 것을 방지합니다.
                ...
                # early stop에서도 forward-backward 흐름을 유지해야 하므로 loss만 잘라 DDP 통신을 유지합니다.
                if stop_ppo:
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            # lm_checkpoint로 critic을 포함한 전체 상태를 저장합니다.
