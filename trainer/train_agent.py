...
# ======== 도구 정의 ========
TOOLS = [
    {"type": "function", "function": {"name": "calculate_math", "description": "수학 표현식을 계산합니다.", "parameters": {"type": "object", "properties": {"expression": {"type": "string"}}, "required": ["expression"]}}},
    ...
    {"type": "function", "function": {"name": "get_current_weather", "description": "날씨를 조회합니다.", "parameters": {"type": "object", "properties": {"location": {"type": "string"}}, "required": ["location"]}}},
    {"type": "function", "function": {"name": "get_current_time", "description": "시간을 조회합니다.", "parameters": {"type": "object", "properties": {"timezone": {"type": "string", "default": "Asia/Seoul"}}, "required": []}}},
...
# ======== mock 데이터 ========
WEATHER_DATA = {"서울": ("28°C", "맑음"), "부산": ("15°C", "흐림"), "광저우": ("32°C", "무더움"), "선전": ("30°C", "맑음"), "항저우": ("22°C", "흐림"), "청두": ("18°C", "가벼운 비"), "우한": ("25°C", "흐림"), "난징": ("20°C", "맑음"), "시안": ("16°C", "강풍"), "충칭": ("26°C", "흐림"), "Tokyo": ("12°C", "맑음"), "New York": ("8°C", "흐림"), "London": ("5°C", "가벼운 비"), "Paris": ("10°C", "흐림"), "Sydney": ("25°C", "맑음")}
TIME_DATA = {"Asia/Seoul": "2025-03-07 14:30:00", "America/New_York": "2025-03-07 01:30:00", "Europe/London": "2025-03-07 06:30:00", "Asia/Tokyo": "2025-03-07 15:30:00", "Europe/Paris": "2025-03-07 07:30:00", "Australia/Sydney": "2025-03-07 17:30:00"}
EXCHANGE_DATA = {("USD", "CNY"): 7.21, ("EUR", "CNY"): 7.85, ("GBP", "CNY"): 9.12, ("JPY", "CNY"): 0.048, ("USD", "EUR"): 0.92, ("USD", "GBP"): 0.79, ("CNY", "JPY"): 20.83, ("AUD", "CNY"): 4.72}
TRANSLATE_DATA = {("안녕하세요 세계", "english"): "Hello World", ("Good morning", "korean"): "좋은 아침입니다", ("오늘 날씨가 정말 좋습니다", "english"): "The weather is nice today", ("I love programming", "korean"): "저는 프로그래밍을 좋아합니다", ("기계 학습은 흥미롭습니다", "english"): "Machine learning is interesting", ("Happy birthday", "korean"): "생일 축하합니다"}
...
MOCK_RESULTS = {
    ...
    "get_current_weather": lambda args: (lambda w: {"city": args.get("location"), "temperature": w[0], "humidity": "65%", "condition": w[1]})(WEATHER_DATA.get(args.get("location"), ("22°C", "맑음"))),
    "get_current_time": lambda args: {"datetime": TIME_DATA.get(args.get("timezone", "Asia/Seoul"), "2025-03-07 14:30:00"), "timezone": args.get("timezone", "Asia/Seoul")},
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    ...
    for turn in range(max_turns):
        ...
        for call in calls:
            ...
            result_str = (json.dumps(result, ensure_ascii=False) if result else '{"error": "tool not found"}')[:2048]  # 너무 큰 숫자가 tokenizer를 과도하게 늘리지 않도록 제한합니다.
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None):
    ...
    for idx, response in enumerate(completions):
        ...
        tool_calls = []
        for turn_answer in turn_answers: tool_calls.extend(parse_tool_calls(turn_answer))  # tool 호출을 파싱합니다.
        reward -= 0.5 * sum(abs(turn.count('<tool_call>') - turn.count('</tool_call>')) for turn in turn_answers)  # 태그 오류 감점
        # -------- 도구 호출 없음: 형식 + reward 점수 --------
        if not tool_calls:
            reward += 0.5 if 5 <= len(response.strip()) <= 800 else -0.5  # 길이 점수
            if '</think>' in response:
                think, answer = response.split('</think>', 1)
                reward += 1.0 if 20 <= len(think.strip()) <= 300 else -0.5  # thinking 길이 점수
                reward += 0.25 if response.count('</think>') == 1 else -0.25  # thinking 종료 태그 점수
                answer = answer.strip()
            ...
            rewards[idx] = max(min(reward, 3.0), -3.0)  # 총점 clipping
        # -------- 도구 호출 있음: 실행 결과 점수 --------
        else:
            ...
            tool_gap = abs(valid_call_count - len(gt)) + max(0, len(tool_calls) - valid_call_count)  # tool 수 차이
            reward += 0.5 if tool_gap == 0 else -0.5 * tool_gap  # tool 정렬 점수
            ...
            if gt: reward += 2.5 * len(verified) / len(gt)  # GT 점수
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_gen_len", type=int, default=768, help="생성 최대 길이")
    parser.add_argument("--max_total_len", type=int, default=2500, help="학습에서 허용할 최종 총 길이 상한")
    parser.add_argument("--data_path", type=str, default="dataset_ko/agent_rl.jsonl", help="학습 데이터 경로")
    parser.add_argument("--num_generations", type=int, default=4, help="prompt당 생성 개수")
    ...
    parser.add_argument("--thinking_ratio", type=float, default=0.1, help="확률적으로 thinking 사용(0.0~1.0)")
