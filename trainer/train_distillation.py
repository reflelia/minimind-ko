...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # forward pass(student 모델)
        with autocast_ctx:
            res = model(input_ids)
            student_logits = res.logits[..., :-1, :].contiguous()

        # teacher 모델 forward pass(eval 및 no_grad에서만 실행)
        if teacher_model is not None:
            with torch.no_grad():
        ...
        # 3) 총 loss = alpha * CE + (1-alpha) * Distill
...
if __name__ == "__main__":
    # 예시는 MoE 모델로 dense 모델을 distillation합니다. 더 큰 teacher_hidden_size 모델로 더 작은 student_hidden_size 모델을 distillation할 수도 있습니다.
    ...
    parser.add_argument('--save_weight', default='full_dist', type=str, help="저장할 가중치 이름")
    parser.add_argument("--epochs", type=int, default=6, help="학습 epoch 수")
    ...
    parser.add_argument("--max_seq_len", type=int, default=340, help="최대 시퀀스 길이")
    parser.add_argument("--data_path", type=str, default="dataset_ko/sft_t2t_mini.jsonl", help="학습 데이터 경로")
    parser.add_argument('--student_hidden_size', default=768, type=int, help="student 모델 hidden size")
    parser.add_argument('--student_num_layers', default=8, type=int, help="student 모델 layer 수")
    parser.add_argument('--teacher_hidden_size', default=768, type=int, help="teacher 모델 hidden size")
    parser.add_argument('--teacher_num_layers', default=8, type=int, help="teacher 모델 layer 수")
    parser.add_argument('--student_use_moe', default=0, type=int, choices=[0, 1], help="student 모델 MoE 사용 여부(0=아니오, 1=예)")
    parser.add_argument('--teacher_use_moe', default=1, type=int, choices=[0, 1], help="teacher 모델 MoE 사용 여부(0=아니오, 1=예)")
    parser.add_argument('--from_student_weight', default='full_sft', type=str, help="student 모델을 시작할 가중치 이름")
    parser.add_argument('--from_teacher_weight', default='full_sft', type=str, help="teacher 모델을 시작할 가중치 이름")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="checkpoint에서 이어 학습할지 여부(0=아니오, 1=예)")
    parser.add_argument('--alpha', default=0.5, type=float, help="CE loss 가중치. 총 loss = alpha*CE + (1-alpha)*KL")
    parser.add_argument('--temperature', default=1.5, type=float, help="생성 또는 distillation temperature")
    ...
    # ========== 5. student와 teacher 모델 정의 ==========
    model, tokenizer = init_model(lm_config_student, args.from_student_weight, device=args.device)
    Logger(f'student 모델 총 파라미터 수：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, device=args.device)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
    Logger(f'teacher 모델 총 파라미터 수：{sum(p.numel() for p in teacher_model.parameters()) / 1e6:.3f} M')
