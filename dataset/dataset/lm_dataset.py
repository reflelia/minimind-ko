...
def pre_processing_chat(conversations, add_system_ratio=0.2):
    # tool use 데이터는 그대로 보존합니다.
    if any(conv.get('tools') for conv in conversations): return conversations

    SYSTEM_PROMPTS = [
        "당신은 지식이 풍부한 AI입니다. 사용자에게 정확한 정보를 제공하세요.",
        "당신은 minimind입니다. 작지만 유용한 언어 모델입니다.",
        "당신은 전문적인 AI 어시스턴트입니다. 가치 있는 답변을 제공하세요.",
...
class DPODataset(Dataset):
    ...
    def __getitem__(self, index):
        sample = self.samples[index]
        chosen = sample['chosen']  # {role, content} 항목들로 이루어진 list입니다.
        rejected = sample['rejected']  # 위와 동일합니다.
...
class RLAIFDataset(Dataset):
    def __init__(self, jsonl_path, tokenizer, max_length=1024, thinking_ratio=0.5):
        ...
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.thinking_ratio = thinking_ratio  # 확률적으로 thinking을 켭니다.
